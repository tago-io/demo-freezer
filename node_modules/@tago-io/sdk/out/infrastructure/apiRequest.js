...
async function apiRequest(axiosObj, cacheTTL) {
    if (cacheTTL) {
        if (RequestInProgress_1.isRequestInProgress(axiosObj)) {
            await sleep_1.default(100);
            return apiRequest(axiosObj, cacheTTL);
        }
        const objCached = Cache_1.getCache(axiosObj);
        if (objCached) {
            return objCached;
        }
    }
    RequestInProgress_1.addRequestInProgress(axiosObj);
    ...
    if (isBrowser_1.default()) {
        ...
        const banner = !process.env.TAGO_RUNTIME
            ...
            : `(Running at TagoIO)`;
        axiosObj.headers = Object.assign(Object.assign({}, axiosObj.headers), { "User-Agent": `TagoIO-SDK|JS|${envParams_json_1.default.version} ${banner}` });
    }
    const request = () => {
    ...
    if (cacheTTL && result && !resulterror) {
        Cache_1.addCache(axiosObj, result, cacheTTL);
    }
    RequestInProgress_1.removeRequestInProgress(axiosObj);
